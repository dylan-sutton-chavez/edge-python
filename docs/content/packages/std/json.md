...
`json` is `loads` and `dumps`. Declare it with `edge add json` and import it by bare name. To pin a different version, point the `edge.json` entry at another URL, see [Modules](/reference/modules#edgejson).
