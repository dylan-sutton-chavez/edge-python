...
`test` is a unit-test harness written in pure Edge Python, not a compiled plugin. Declare it with `edge add test` and import it by bare name. To pin a different version, point the `packages.json` entry at another URL, see [Modules](/reference/modules#packagesjson).
