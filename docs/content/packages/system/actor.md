...
`actor` is message passing inside an [actor pool](/reference/actors). Declare it with `edge add actor` and import it by bare name. The CLI builds it in and maps the entry's CDN URL to that implementation, and it is meaningful only to programs running under `edge actor`. The JS host loads the CDN stub the entry points at, and its `send` throws `actor.send needs the CLI` at the first call.
