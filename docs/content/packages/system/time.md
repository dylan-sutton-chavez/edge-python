...
`time` is clocks and calendar functions. Declare it with `edge add time` and import it by bare name. It runs in the JS host and in the CLI, which builds it in and maps the entry's CDN URL to that implementation, see [The CLI](/reference/modules#the-cli).
