...
`storage` is key-value storage and IndexedDB from scripts. It is a JavaScript module on the browser's main thread. Import it by bare name or declare it in the `host` field of `packages.json`. The native engine rejects `import storage` at compile time, see [the native engine](/reference/modules#the-native-engine).
