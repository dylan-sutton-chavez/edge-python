...
Plain-JS capabilities that run on the browser's main thread. Register them declaratively, programmatically, or not at all — see [How to load them](#how-to-load-them).

No `.wasm`, no Rust, no build step. Each call defers to the main thread over `postMessage` (cost figures in [Writing modules](/reference/writing-modules#choosing-between-the-three-paths)). Python sees a synchronous call. The ESM loads lazily, the first time a run imports it.
