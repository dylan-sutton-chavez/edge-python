...
- **Call-time name propagation**: free names in a function body fill from the caller's frame at call time through per-`(caller chunk, callee)` cached slot maps, then a bare-name fallback (caller SSA -> callee module attrs -> globals -> entry-module state).
