...
The CLI keeps the official packages inside the binary, so the entry needs no network there. The package catalog lives in [Modules](/docs/reference/modules#standard-packages) and the manifest format in [edge.json](/docs/reference/modules#edgejson).
