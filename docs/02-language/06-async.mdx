...
In the JS host, `fetch_text` runs the platform `fetch()` inside a Web Worker, and a browser subjects it to CORS. The CLI runs the same calls on host threads, so `gather` overlaps them there too, with no CORS.
