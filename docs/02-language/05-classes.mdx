...
- Augmented-assignment dunders (`__iadd__`, ...). `a += b` desugars to `a = a + b`, so `__add__` covers it. Exception: list `+=` extends in place (alias-visible). See [Data types](/docs/language/data-types#list).
- Async dunders (`__aenter__` / `__aexit__` / `__aiter__` / `__anext__`). `async with` and `async for` use the sync paths. See [Async](/docs/language/async).
