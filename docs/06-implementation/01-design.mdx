...
Deep dives live in their own pages. Tokenization is in [Lexical](/docs/implementation/lexical). Parsing, SSA, and bytecode emission are in [Parsing](/docs/implementation/parsing). This page covers the parts that span the pipeline.
...
- **Template memoisation**: pure user functions cache `(args) -> result` after `TPL_THRESH = 2` hits, capped at 256 entries per function. A table self-disables and frees after 256 consecutive lookup misses. Caching requires a call without keywords, immutable arguments (mutable containers disqualify), a pure body, and free names limited to builtins and the function itself. Static purity is computed at parse time (see [Parsing](/docs/implementation/parsing#lambda-and-function-bodies)) and backed by a runtime check that propagates effects through calls, so a statically pure wrapper over an impure callee such as `apply(print, x)` is never cached. Hashing is an FNV fold over the raw `Val` bits with a value-equality verify.
...
Each `Instruction` is 4 bytes. That is a 1-byte `OpCode` (`#[repr(u8)]`), a 2-byte operand, and 1 byte of padding. About 40 specialised `Call*` variants cover hot builtins. Operand meanings per opcode are listed in [Parsing](/docs/implementation/parsing#bytecode-model).
...
The heap is a `Vec<HeapSlot>` arena with a free list capped at 524,288 entries, sorted to prefer low indices. Strings and bytes up to 128 bytes, all LongInts, and bound methods are interned in side hashes. Equal values collapse to one slot, so short literals short-circuit through identity (`is`). The live-object cap is `Limits.heap` (see [Limits and errors](/docs/reference/limits-and-errors#sandboxed-execution)).
...
`async def` and `yield`-bearing `def` both compile to `HeapObj::Coroutine`. The user-facing primitives are described in [Async](/docs/language/async).
...
`vm.run()` wraps the module body as an implicit coroutine, so top-level statements suspend like `async def` bodies. Dispatch is single-driver. `top_loop` is the only place that picks coroutines.

`run`, `gather`, `with_timeout`, `await`, and calling a coroutine value push targets to the scheduler, park the caller in `CoroState::WaitingForChildren`, and yield. The `WaitKind` picks the finalize behavior. `Run(target)` returns the target's value, `Gather` returns the list of results, `Timeout` enforces a deadline.

When children finish, the outer's saved stack placeholder is spliced with the result and the outer is marked ready. A child's error, a timeout or a failed host call marks it `CoroState::Raising` instead, and the next resume raises the error at the park point, so handlers and the traceback position follow the normal path.
...
Chunk-derived tables (bytecode, name pools, the extern table) are not stored. They come from the re-parse, so only dynamic state crosses the wire.

Restore runs in two passes because hashing reads the heap. First every slot is materialised (sets and frozensets land empty), then a rehash pass rebuilds dict indexes and fills the sets, and `rebuild_mro` recomputes each class linearization. IC and memoisation caches start empty and warm lazily, and `Extern` handles resolve by name against the re-parsed chunk's extern table.

Host-side resources such as in-flight host calls and DOM handles are not part of the snapshot. `state_globals` and `state_stack` introspect a parked run without resuming it.

See [Snapshots](/docs/language/snapshots) for the host-facing feature and the [ABI](/docs/reference/abi#snapshot-exports) for the exports and blob layout.
...
- No runtime module system. Imports resolve at parse time through a host-injected resolver. See [Modules](/docs/reference/modules).
- No bigints, complex numbers, `bytearray`, `memoryview`, `Decimal`, or `Fraction`. No `gen.send` / `throw` / `close`. No `asyncio` module. Concurrency primitives are top-level builtins (see [Async](/docs/language/async)).
