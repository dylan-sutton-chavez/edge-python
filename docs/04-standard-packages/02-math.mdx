...
`math` is scalar math on `libm`, with `ValueError: math domain error` for domain errors. Declare it with `edge add math` and import it by bare name. To pin a different version, point the `edge.json` entry at another URL, see [Modules](/docs/reference/modules#edgejson).
...
The batch functions work on a `bytes` buffer of little-endian f64 values. Pack it with [struct](/docs/standard-packages/struct):
