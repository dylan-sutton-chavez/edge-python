...
`dom` wraps the browser DOM. It is a JavaScript module that needs a browser, it runs on the main thread where `document` and `window` live. Declare it with `edge add dom`, an `imports` entry pointing at the facade `https://cdn.edgepython.com/js/builtins/dom/entry.py`, and import it by bare name. A JavaScript runtime without a page loads it and fails at the first call with `module 'dom' needs 'document', missing in this runtime`, raised in the calling coroutine. The CLI loads it the same way and names the JavaScript half behind the facade, `module '_dom' needs 'document', missing in this runtime`, see [JavaScript in the CLI](/docs/reference/modules#javascript-in-the-cli).
