...
`network` is HTTP, WebSocket, and Server-Sent Events from scripts. Declare it with `edge add network` and import it by bare name. It runs in the JS host, in a browser or elsewhere, and in the CLI, which builds it in and maps the entry's CDN URL to it, see [The CLI](/docs/reference/modules#the-cli).
