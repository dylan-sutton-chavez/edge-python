...
const RUN_TIMEOUT_MS = 10000 // hard per-run wall-clock cap so a hung snippet cannot wedge the page queue

let workerPromise = null
let workerReady = false // flips true once the worker and wasm load, it gates the cold-start phases
let activeSink = null // raw-stdout-chunk handler of the block currently running
let runChain = Promise.resolve() // serializes runs, one shared worker and one sink cannot host two blocks at once
...
function getWorker(onPhase) {
	...
	workerPromise = (async () => {
		...
		onPhase?.('worker')
		const worker = await createWorker({ wasmUrl: WASM_URL, integrity: true, imports: IMPORTS })
...
export async function run(src, onChunk, onPhase) {
	const exec = async () => {
		...
		try {
			...
			runP.catch(() => {}) // the race loser, killWorker rejects it with nobody listening
