...
`lang` is licensed apart from the engine, so using it needs a written agreement, which [c.sutton.dylan@gmail.com](mailto:c.sutton.dylan@gmail.com) arranges. The engine itself stays Apache 2.0, so a host that drives `compiler.wasm` through the [ABI](/docs/reference/abi) needs nothing from anyone.
...
- [Limits and errors](/docs/reference/limits-and-errors) details each cap and the errors they raise.
- [Modules](/docs/reference/modules) covers the resolver and how imports reach the engine.
