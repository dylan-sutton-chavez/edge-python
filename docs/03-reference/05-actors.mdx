...
An `eval` group runs code it does not trust. Each incoming message is compiled and run in a fresh wasm instance with its own linear memory, capped by the group's `heap` limit inside the interpreter and by a 256 MiB reservation outside it. A deadline the host enforces from outside the instance cuts the run off after ten seconds of wall-clock time, so a runaway loop or a long native operation ends even where the interpreter cannot yield.

Nothing survives between messages, the instance is dropped when the run ends. A bundle that carries its own `edge.json` resolves through it, any other message through the pool's manifest.
...
- [CLI](/docs/reference/cli) for `edge actor` and `edge build`.
- [Async](/docs/language/async) for the cooperative scheduler each actor runs on.
- [Snapshots](/docs/language/snapshots) for freezing and resuming a single run.
