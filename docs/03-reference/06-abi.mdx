...
The reference bridge is `js/src/native.ts` in the repo, and the [CLI](/docs/reference/modules#the-cli) implements the same six imports in Rust over wasmtime, bridging the plugin instance and the compiler instance the same way. The CLI compiles a plugin with Cranelift on first use and caches the machine code. Rust embedders mirror the shape.
...
- **ABI v1 leaks about 8 bytes per host call** in guest linear memory. A single worker session caps at roughly 500 k plugin calls, so recycle the worker periodically for unbounded streaming.
## Author conveniences
...
`restore_state` re-parses the embedded source, recomputes the fingerprint, and rejects any blob whose fingerprint does not match the freshly compiled chunk. This pins each blob to one program and one compiler build. A blob has no size cap. The serializer is `src/vm/snapshot.rs` in the repo, with internals in [Design](/docs/implementation/design).
...
To add native modules from a Rust host, implement the `Resolver` trait. See [Modules](/docs/reference/modules).

## See also

- [Modules](/docs/reference/modules): import resolution on the script side, edge.json, integrity verification, and the delivery paths.
- [Snapshots](/docs/language/snapshots): freezing and resuming a paused run from the host.
