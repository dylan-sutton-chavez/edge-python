...
Every execution is metered, there is no unmetered mode. Both hosts, the JS host and the CLI, run every script under these limits, and an embedder overrides them per field, `createWorker` through its `limits` option and `edge actor` through a group's.
...
Import failures are compile-time diagnostics too, including modules Edge Python does not ship (`os`, `sys`, `asyncio`). See [Modules](/docs/reference/modules#resolution-errors) for those message formats.
...
User-defined classes do not join the built-in tree, but they support inheritance among themselves: `except UserBase` catches a raised `UserSub` when `UserSub` inherits from `UserBase`. For `raise X from Y` and chaining, see [Control flow](/docs/language/control-flow).
