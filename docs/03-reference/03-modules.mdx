...
| Native module | `.wasm` | A plugin over the [ABI](/docs/reference/abi). A host can also register bindings of this kind through the `Resolver` trait. |
| JS module | `.js`, `.mjs` | Plain ESM the JS host runs on the page's main thread and the CLI runs in StarlingMonkey, see [JS module](#js-module). |
...
Dots map to directories and the `.py` suffix is implicit. A leading dot anchors the spec at the importing file, a dotted name anchors at the nearest `edge.json` directory, and a plain name must be declared in `edge.json`.

That includes the official packages, nothing resolves without a manifest entry, and `edge add <name>` writes the entry for each official name. The CLI keeps embedded copies of the standard packages, so their entries need no network there.

Two forms do not work. `from . import x` names nothing, because Edge has no packages for a bare dot to reach. Dynamic imports are absent too, there is no `__import__` and no `importlib`, and the module set is fixed per compilation.
...
- `name`, `version`, `description`, `repository`, `docs`: what a package carries into a registry. The compiler ignores all five, the CLI checks their shape. A name is lowercase letters, digits and single hyphens starting with a letter, a version is `major.minor.patch` with digits only, a description is one line of 60 characters at most, a repository is an `https` url a listing can link, and `docs` is a relative directory inside the project.

Only what nothing else can supply belongs here. An author and a date come from the account that publishes, and the license is read from the `LICENSE` file the artifact already carries, so none of the three is a field.
...
Only `sha256` is supported. Other prefixes fail with `unrecognized integrity fragment`. The JS host checks the pin in its fetch layer and refuses one on a `.js` or `.mjs` module, which the page imports itself and cannot hash. The CLI checks the pin on every module it downloads or reads from disk, `.wasm` plugins and JavaScript modules included, where the pin covers the module's entry file. It does not read the pin on the standard package URLs it serves from inside the binary.
...
- [json](/docs/standard-packages/json), [math](/docs/standard-packages/math), [re](/docs/standard-packages/re), [struct](/docs/standard-packages/struct), [test](/docs/standard-packages/test)

System libraries:

- [dom](/docs/system-libraries/dom), [network](/docs/system-libraries/network), [storage](/docs/system-libraries/storage), [time](/docs/system-libraries/time)

None of them resolves on its own. A project declares each one it uses in `imports`, and `edge add` writes the entry. A standard package points at `https://cdn.edgepython.com/std/<name>.wasm` (`test` ships as `test.py`), a JavaScript library at `https://cdn.edgepython.com/js/builtins/<name>/index.js`, and `dom` at its facade `https://cdn.edgepython.com/js/builtins/dom/entry.py`.

Where each one runs follows from that artifact. A `.wasm` or `.py` runs inside the engine, a `.js` on the JS host's main thread or in the CLI's [JavaScript runtime](#javascript-in-the-cli).

The JS host fetches those URLs and caches them. The CLI resolves the standard package URLs to the copies embedded in the binary and downloads the JavaScript libraries once into its cache, so the same manifest serves both hosts. A page that drives `createWorker` directly passes the same entries through `imports`, or a `mainThreadModules` object for an in-page module.
...
| JS module | Plain ESM declared in `imports` by URL, run on the JS host's main thread or in the CLI's StarlingMonkey | JavaScript | Transit values plus the host globals, `window` and `document` in a browser |

Transit values are `None`, `bool`, `int` (128-bit), `float`, `str`, `bytes`, and nested `list` / `dict`. The exact wire tags are in the [ABI](/docs/reference/abi).

### CDN wasm

The contract is the [ABI](/docs/reference/abi), language-agnostic and sealed. Rust authors use the `wasm-pdk` crate's macros (`#[plugin_fn]`, `#[plugin_class]`, and friends) and write plain Rust. Other languages use community PDKs or hand-written wire boilerplate. The script side imports it through a manifest alias:
...
A module that fails to load fails at compile time, at the import that names it, with ` (via <importer>)` appended when another module reached it.
...
| A JavaScript module that throws while loading | `module 'charts' failed to load, Error: <message>` |
| A JavaScript module that imports above its directory | `module 'charts' imports '../x.js' from outside its directory` |
| A plugin, or a URL off the official CDN, in an [eval group](#untrusted-code) | `module 'fastmath' is not available to untrusted eval runs` |

### JavaScript in the CLI

The CLI runs every `.js` and `.mjs` module in [StarlingMonkey](https://github.com/bytecodealliance/StarlingMonkey), SpiderMonkey compiled to a WebAssembly component, under the same wasmtime that runs the engine. It is the only JavaScript engine the CLI has. Each module gets its own runtime, started on first import and shared by every program of one process that imports it, and calls, Promises and `pushEvent` behave as they do in the JS host. A module's console output goes to stdout, `console.log` lines as written and the other levels behind their name, such as `Warn: `.

A module runs from a read-only copy of its directory, the entry and every file it reaches through `./` or `../` imports. It cannot import a bare package or a URL, and it sees no environment variables and no files outside that copy.

The runtime has the Web APIs a server-side worker has, `fetch` with streams and `AbortController`, `setTimeout` and `setInterval`, `TextEncoder` and `TextDecoder`, `URL`, `crypto`, `performance`, `structuredClone`, `atob` and `btoa`. It has no `Intl`, `WebSocket`, `EventSource`, `MessageChannel`, `URLPattern` or `WebAssembly`, and nothing that belongs to a page, such as `document`, `window`, `localStorage` or `indexedDB`. Its time zone is always UTC. A module that reaches a missing global fails at that call with the JS host's text, so for the official libraries

- `time` runs, except `tzname()`, which raises `module 'time' needs 'Intl', missing in this runtime`, and `localtime` equals `gmtime`,
- `network` runs `fetch`, `fetch_text`, `fetch_json`, `abort_request` and the Server-Sent Events calls, while `ws_open` raises `module 'network' needs 'WebSocket', missing in this runtime`,
- `storage` and `dom` import and raise at their first call, `module 'storage' needs 'localStorage', missing in this runtime`.

The runtime is not part of the binary. The first JavaScript import downloads it once from `https://cdn.edgepython.com/js-runtime/<sha256>.cwasm`, precompiled for the platform, about 9 MB compressed and 30 MB on disk. The CLI checks the bytes against the SHA-256 built into the binary and keeps them in `~/.cache/edge/js-runtime`, and later runs load them in a few milliseconds.

An app binary packed from a project that declares a JavaScript module carries its own copy, which a run prefers over the cache and the download, so a packed app stays offline. When no copy is at hand and the download fails, the import fails with `module 'time' needs the JavaScript runtime, fetching '<url>' failed, <reason>`.

A runtime's memory is capped at 1 GiB, or at the run's own cap when that is lower, and allocating past it raises `out of memory` in the calling coroutine. A call that does not answer within 30 seconds, or before an eval run's deadline, stops the runtime with `module 'x' did not answer within 30 s`, and the next call starts a fresh one.
...
| `--save-state <f>` | On a wait the engine cannot serve, write a [snapshot](/docs/language/snapshots) to the file and exit 0. |
...
Code you do not trust goes to an [eval group](/docs/reference/actors#untrusted-code), where each message runs in a fresh wasm instance with a memory cap and a ten second wall-clock deadline. A bundle that carries its own `edge.json` resolves through it, any other message through the pool's manifest. Either way `.wasm` plugins are refused, remote modules load only from `https://cdn.edgepython.com/`, and the JavaScript runtime has no network, so `network.fetch` fails with `NetworkError when attempting to fetch resource`. `send()` has no scheduler there.
...
The build also needs the pinned StarlingMonkey release in `target/starling.wasm`, or at the path `EDGE_STARLING_WASM` names, see `CONTRIBUTING.md` for the command. `cli/build.rs` reads the artifacts from those paths, or from `EDGE_COMPILER_WASM` and `EDGE_STD_DIR`, fetches nothing, and precompiles StarlingMonkey into the artifact the binary later downloads by hash.

## See also

- [ABI](/docs/reference/abi): the wire contract behind `.wasm` plugins.
- [CLI](/docs/reference/cli): `edge add` writes manifest entries, `edge build` vendors packages for offline use.
- [Limits and errors](/docs/reference/limits-and-errors): the sandbox profile both hosts run under.
