...
- `name`, `version`, `description`, `docs`: what a package carries into a registry. The compiler ignores all four, the CLI checks their shape. A name is lowercase letters, digits and single hyphens starting with a letter, a version is `major.minor.patch` with digits only, a description is one line of 60 characters at most, and `docs` is a relative directory inside the project. An author and a date are not fields here, a publish reads those from the account.
