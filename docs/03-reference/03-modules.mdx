...
Nothing resolves without a manifest entry, and `edge add <name>` writes the entry.
...
Only `sha256` is supported. Other prefixes fail with `unrecognized integrity fragment`. The JS host checks the pin in its fetch layer and refuses one on a `.js` or `.mjs` module, which the page imports itself and cannot hash. The CLI checks the pin on every module it downloads or reads from disk, `.wasm` plugins and JavaScript modules included, where the pin covers the module's entry file.
...
The first diagnostic is the same for a typo and for a package you forgot to declare. The CLI adds a `help:` line pointing at `edge add <name>` or at a relative import. It also covers modules Edge Python does not ship, like `os` or `sys`. They parse for syntactic compatibility and are then rejected here, before any code runs.
...
The CLI compiles a `.wasm` plugin from disk or a URL with Cranelift on first use and keeps the machine code in `~/.cache/edge/plugins`, keyed by the file's hash, so later runs skip the compile.
...
cd cli && cargo build --release   # embeds it precompiled
```

The build also needs the pinned StarlingMonkey release in `target/starling.wasm`, or at the path `EDGE_STARLING_WASM` names, see `CONTRIBUTING.md` for the command. `cli/build.rs` reads the artifacts from those paths, or from `EDGE_COMPILER_WASM`, fetches nothing, and precompiles StarlingMonkey into the artifact the binary later downloads by hash.
