...
// A message a node emitted, drained by the scheduler after each run step.
pub struct Outgoing {
    pub group: String,
    pub body: String,
}
...
// Drains everything a node sent during its last run step.
pub fn drain_outbox() -> Vec<Outgoing> {
    OUTBOX.with(|o| core::mem::take(&mut *o.borrow_mut()))
}

/* Message passing between swarm nodes, send is fire and forget over the scheduler. */
pub(super) fn bindings() -> Vec<NativeBinding> {
    vec![NativeBinding::from_fn("send", send, false)]
}

// Queues a message to a group, delivered to one of its nodes round robin.
fn send(heap: &mut HeapPool, args: &[Val], _: Option<Val>) -> Result<Val, VmErr> {
    let group = str_arg(heap, args, 0, "swarm.send")?;
    let body = str_arg(heap, args, 1, "swarm.send")?;
