...
impl Barrier {
    fn new(total: usize) -> Self {
        Barrier { inflight: AtomicUsize::new(0), total, idle: Mutex::new(Quiescence { idle: 0, shutdown: false }), changed: Condvar::new() }
    }
    ...
    /* Parks an idle shard until work arrives or the whole pool idles, true means shut down. */
...
// Runs the pool across `threads` schedulers, groups sharded round robin over threads.
pub fn run(config: ActorConfig, threads: usize) -> i32 {
    let threads = threads.max(1);
    let runtime = match runtime(&config) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("error: {e}");
            return 1;
    ...
    // One thread keeps the simple single-thread path, no channels or router needed.
    if threads == 1 {
        return match Scheduler::new(config, runtime) {
            Ok(mut s) => s.run(),
            Err(e) => {
                eprintln!("error: {e}");
                1
    ...
    for (groups, rx) in shard_groups.into_iter().zip(receivers) {
        ...
        handles.push(std::thread::spawn(move || match Scheduler::new(shard_config, runtime) {
