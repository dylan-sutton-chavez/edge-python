...
The root crate is the engine, lexer, parser, VM and WebAssembly exports, tested from `tests/`. `cli/` is the `edge` binary that runs `compiler.wasm` under wasmtime, and `js/` is the JavaScript host with its builtin modules. `std/` holds the standard packages, `pdk/` and `abi/` are the kit for writing `.wasm` plugins, and `lang/` builds your own scripting language on the engine. `site/` is the website that serves `docs/`, `infra/` declares its Cloudflare resources in code, and `fuzz/` and `skill/` hold the fuzzer and a guided reference for AI models. Build and test commands for every part live in [CONTRIBUTING.md](CONTRIBUTING.md).
